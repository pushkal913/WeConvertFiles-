...
const desktopLinks = orderedTools
  .map((tool) => `                <a class="${desktopLinkClass}" href="${guideHrefForTool(tool.id)}">${escapeHtml(tool.title)} Guide</a>`)
  .join('\n');

const mobileGroups = categories.map((category) => {
  const links = category.toolIds.map((id) => {
    const tool = toolById.get(id);
    return `                    <a class="${mobileLinkClass}" href="${guideHrefForTool(tool.id)}">${escapeHtml(tool.title)}</a>`;
  }).join('\n');

  return `                <details class="group border border-slate-200 dark:border-slate-700/60 bg-slate-50 dark:bg-slate-900/60 rounded-2xl overflow-hidden">
                  ...
                  <div class="p-3 bg-white dark:bg-[#1e293b] border-t border-slate-200/60 dark:border-slate-700/60 grid grid-cols-2 gap-2">
${links}
                  </div>
                </details>`;
}).join('\n');
...
const generatedBlock = `              <!-- GENERATED_GUIDE_GRIDS_START -->
              <div data-guide-view="desktop" class="mt-5 hidden md:grid gap-3 md:grid-cols-3 xl:grid-cols-4">
${desktopLinks}
              </div>
